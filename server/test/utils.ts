...
const withDatabase = (url: string, name: string) => url.replace('/fat', `/${name}`);
...
export const getKyselyDB = async (suffix?: string): Promise<Kysely<DB>> => {
  ...
  const dbName = `fat_${suffix ?? randomSuffix}`;
  await sql.unsafe(`CREATE DATABASE ${dbName} WITH TEMPLATE OWNER postgres;`);
