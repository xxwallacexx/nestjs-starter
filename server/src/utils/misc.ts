...
import { AppCookie, AppHeader, MetadataKey } from 'src/enum';
...
export const getExternalDomain = (server: SystemConfig['server']) =>
  server.externalDomain || `https://my.fat.app`;
...
export const useSwagger = (app: INestApplication, { write }: { write: boolean }) => {
  const config = new DocumentBuilder()
    .setTitle('FetchAppTech')
    .setDescription('FetchAppTech API')
    ...
    .addCookieAuth(AppCookie.ACCESS_TOKEN)
    .addApiKey(
      {
        type: 'apiKey',
        in: 'header',
        name: AppHeader.API_KEY,
      },
  ...
  const customOptions: SwaggerCustomOptions = {
    ...
    customSiteTitle: 'FetchAppTech API Documentation',
  };
  ...
  if (write) {
    ...
    const outputPath = path.resolve(process.cwd(), '../open-api/fat-openapi-specs.json');
