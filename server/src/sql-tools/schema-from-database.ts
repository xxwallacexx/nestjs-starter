    ...
    switch (constraint.constraint_type) {
      ...
      case 'f': {
        if (
          ...
          !constraint.reference_column_names
        ) {
          warn(
  ...
  const items = await db
    ...
    .innerJoin('pg_namespace', (join) =>
      join
        ...
        .on('pg_namespace.nspname', '=', schemaName),
    )
    ...
    .select((eb) => [
      ...
      jsonArrayFrom(
        eb
          .selectFrom('pg_enum as e')
          .select(['e.enumlabel as value'])
          .whereRef('e.enumtypid', '=', 'pg_type.oid'),
      ).as('values'),
    ])
    .execute();
...
const getTableColumns = (db: DatabaseClient, schemaName: string) => {
  return db
    ...
    .leftJoin('pg_type as t', (join) =>
      join
        .onRef('t.typname', '=', 'c.udt_name')
        .on('c.data_type', '=', sql.lit('USER-DEFINED')),
    )
  ...
  return (
    db
      ...
      .select((eb) => [
        ...
        'a.amname as using',
        eb
          .fn<string>('pg_get_expr', ['ix.indexprs', 'ix.indrelid'])
          .as('expression'),
        ...
        eb
          ...
          .select((eb) =>
            eb.fn<string[]>('json_agg', ['a.attname']).as('column_name'),
          )
          .as('column_names'),
      ])
...
const getTableConstraints = (db: DatabaseClient, schemaName: string) => {
  return db
    ...
    .innerJoin('pg_class as source_table', (join) =>
      join
        .onRef('source_table.oid', '=', 'pg_constraint.conrelid')
        .on('source_table.relkind', 'in', [
          // ordinary table
          sql.lit('r'),
          // partitioned table
          sql.lit('p'),
    ...
    .select((eb) => [
      ...
      eb
        ...
        .whereRef(
          'pg_attribute.attnum',
          '=',
          sql`any("pg_constraint"."conkey")`,
        )
        .select((eb) =>
          eb
            .fn<string[]>('json_agg', ['pg_attribute.attname'])
            .as('column_name'),
        )
        .as('column_names'),
      eb
      ...
      eb
        .fn<string>('pg_get_constraintdef', ['pg_constraint.oid'])
        .as('expression'),
    ])
    .where('pg_namespace.nspname', '=', schemaName)
    .execute();
