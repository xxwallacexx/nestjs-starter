import {
  getColumnType,
  schemaDiffToSql,
} from 'src/sql-tools/schema-diff-to-sql';
import {
...
const isSynchronizeDisabled = (
  source?: { synchronize?: boolean },
...
export const schemaDiff = (
  ...
  return {
    items,
    asSql: (options?: SchemaDiffToSqlOptions) =>
      schemaDiffToSql(items, options),
  };
};
...
export const diffTables = (
  ...
  const sourceMap = Object.fromEntries(
    sources.map((table) => [table.name, table]),
  );
  const targetMap = Object.fromEntries(
    targets.map((table) => [table.name, table]),
  );
...
const diffColumns = (
  sources: DatabaseColumn[],
  targets: DatabaseColumn[],
): SchemaDiff[] => {
  const items: SchemaDiff[] = [];
  const sourceMap = Object.fromEntries(
    sources.map((column) => [column.name, column]),
  );
  const targetMap = Object.fromEntries(
    targets.map((column) => [column.name, column]),
  );
  const keys = new Set([...Object.keys(sourceMap), ...Object.keys(targetMap)]);
...
const diffColumn = (
  ...
  if (isTypeChanged) {
    ...
    return dropAndRecreateColumn(
      source,
      target,
      `column type is different (${sourceType} vs ${targetType})`,
    );
  }
...
const diffConstraints = (
  sources: DatabaseConstraint[],
  targets: DatabaseConstraint[],
): SchemaDiff[] => {
  const items: SchemaDiff[] = [];
...
const diffConstraint = <T extends DatabaseConstraint>(
  source?: T,
  target?: T,
): SchemaDiff[] => {
  if (isSynchronizeDisabled(source, target)) {
  ...
  switch (source.type) {
    case DatabaseConstraintType.PRIMARY_KEY: {
      return diffPrimaryKeyConstraint(
        source,
        target as DatabasePrimaryKeyConstraint,
      );
    }

    case DatabaseConstraintType.FOREIGN_KEY: {
      return diffForeignKeyConstraint(
        source,
        target as DatabaseForeignKeyConstraint,
      );
    }
    ...
    default: {
      return dropAndRecreateConstraint(
        source,
        target,
        `Unknown constraint type: ${(source as any).type}`,
      );
    }
  }
};
...
const diffUniqueConstraint = (
  source: DatabaseUniqueConstraint,
  target: DatabaseUniqueConstraint,
): SchemaDiff[] => {
  let reason = '';
...
const diffCheckConstraint = (
  source: DatabaseCheckConstraint,
  target: DatabaseCheckConstraint,
): SchemaDiff[] => {
...
const diffIndexes = (sources: DatabaseIndex[], targets: DatabaseIndex[]) => {
  const items: SchemaDiff[] = [];
  const sourceMap = Object.fromEntries(
    sources.map((index) => [index.name, index]),
  );
  const targetMap = Object.fromEntries(
    targets.map((index) => [index.name, index]),
  );
  const keys = new Set([...Object.keys(sourceMap), ...Object.keys(targetMap)]);
...
const diffIndex = (
  ...
  if (source && !target) {
    return [
      { type: 'index.create', index: source, reason: Reason.MissingInTarget },
    ];
  }
