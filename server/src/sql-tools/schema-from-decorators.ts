...
type RegisterItem =
  ...
  | {
      type: 'columnIndex';
      item: PropertyBased<{ options: ColumnIndexOptions }>;
    }
  | {
...
const asSnakeCase = (name: string): string =>
  name.replaceAll(/([a-z])([A-Z])/g, '$1_$2').toLowerCase();
const asKey = (prefix: string, tableName: string, values: string[]) =>
  (prefix + sha1(`${tableName}_${values.toSorted().join('_')}`)).slice(0, 30);
const asPrimaryKeyConstraintName = (table: string, columns: string[]) =>
  asKey('PK_', table, columns);
const asForeignKeyConstraintName = (table: string, columns: string[]) =>
  asKey('FK_', table, columns);
const asRelationKeyConstraintName = (table: string, columns: string[]) =>
  asKey('REL_', table, columns);
const asUniqueConstraintName = (table: string, columns: string[]) =>
  asKey('UQ_', table, columns);
const asCheckConstraintName = (table: string, expression: string) =>
  asKey('CHK_', table, [expression]);
const asIndexName = (
  table: string,
...
const makeColumn = ({
  ...
  const enumName =
    options.enumName ?? `${tableName}_${columnName}_enum`.toLowerCase();
...
const missingTableError = (
  ...
  const label =
    object.constructor.name + (propertyName ? '.' + String(propertyName) : '');
...
export const schemaFromDecorators = () => {
  if (!initialized) {
    ...
    for (const { item } of items.filter(
      (item) => item.type === 'foreignKeyColumn',
    )) {
      processForeignKeyColumn(schemaTables, item, { warn });
    }

    for (const { item } of items.filter(
      (item) => item.type === 'uniqueConstraint',
    )) {
      processUniqueConstraint(schemaTables, item, { warn });
    }

    for (const { item } of items.filter(
      (item) => item.type === 'checkConstraint',
    )) {
...
const processTable = (
  tables: SchemaTables,
  { object, options }: ClassBased<{ options: TableOptions }>,
) => {
  ...
  if (!options.primary && options.unique) {
    table.constraints.push({
      ...
      name:
        ...
        asUniqueConstraintName(table.name, [column.name]),
      tableName: table.name,
...
const processPrimaryKeyConstraint = (table: SchemaTable) => {
  ...
  if (columnNames.length > 0) {
    table.constraints.push({
      ...
      name:
        table.options.primaryConstraintName ||
        asPrimaryKeyConstraintName(table.name, columnNames),
      tableName: table.name,
      columnNames,
...
const processIndex = (
  ...
  table.indexes.push({
    name:
      options.name || asIndexName(table.name, options.columns, options.where),
    tableName: table.name,
...
const processColumnIndex = (
  tables: SchemaTables,
  {
    ...
    propertyName,
    options,
  }: PropertyBased<{ options: ColumnIndexOptions }>,
  { warn }: { warn: OnWarn },
) => {
  const table = resolveTable(tables, object.constructor);
  if (!table) {
  ...
  const column = findByName(
    table.columns,
    Reflect.getMetadata(SchemaKey.ColumnName, object, propertyName),
  );
  if (!column) {
    return;
  }
...
const processForeignKeyColumn = (
  tables: SchemaTables,
  {
    ...
    propertyName,
    options,
  }: PropertyBased<{ options: ForeignKeyColumnOptions; target: () => object }>,
  { warn }: { warn: OnWarn },
) => {
  const table = resolveTable(tables, object.constructor);
  if (!table) {
...
const processForeignKeyConstraint = (
  ...
  const referenceColumns = parentTable.columns.filter(
  ...
  childTable.constraints.push({
    name:
      ...
      asForeignKeyConstraintName(childTable.name, columnNames),
  ...
  if (options.unique) {
    childTable.constraints.push({
      name:
        options.uniqueConstraintName ||
        asRelationKeyConstraintName(childTable.name, columnNames),
      tableName: childTable.name,
      columnNames,
      type: DatabaseConstraintType.UNIQUE,
      synchronize: options.synchronize ?? true,
