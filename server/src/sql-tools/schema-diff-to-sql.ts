...
const asColumnList = (columns: string[]) =>
  columns
    ...
    .map((column) => `"${column}"`)
    .join(', ');
const withNull = (column: DatabaseColumn) =>
  column.nullable ? '' : ' NOT NULL';
const withDefault = (column: DatabaseColumn) =>
  column.default ? ` DEFAULT ${column.default}` : '';
const withAction = (constraint: {
 ...
 */
export const schemaDiffToSql = (
  items: SchemaDiff[],
  options: SchemaDiffToSqlOptions = {},
): string[] => {
  return items.flatMap((item) =>
    asArray(asSql(item)).map(
      (result) => result + withComments(options.comments, item),
    ),
  );
};
...
const asConstraintAdd = (constraint: DatabaseConstraint): string | string[] => {
  ...
  switch (constraint.type) {
    ...
    case DatabaseConstraintType.FOREIGN_KEY: {
      const columnNames = asColumnList(constraint.columnNames);
      const referenceColumnNames = asColumnList(
        constraint.referenceColumnNames,
      );
      return (
