...
export class TelemetryRepository {
  ...
  constructor(
    ...
    this.api = new MetricGroupRepository(metricService).configure({
      enabled: metrics.has(AppTelemetry.API),
    });
    this.host = new MetricGroupRepository(metricService).configure({
      enabled: metrics.has(AppTelemetry.HOST),
    });
    this.jobs = new MetricGroupRepository(metricService).configure({
      enabled: metrics.has(AppTelemetry.JOB),
    });
    this.repo = new MetricGroupRepository(metricService).configure({
      enabled: metrics.has(AppTelemetry.REPO),
    });
  }
