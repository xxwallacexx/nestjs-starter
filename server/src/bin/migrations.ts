#!/usr/bin/env node
process.env.DB_URL = process.env.DB_URL || 'postgres://postgres:postgres@localhost:5432/fat';
...
const create = (path: string, up: string[], down: string[]) => {
  ...
  const fullPath = join(folder, 'migrations', filename);
