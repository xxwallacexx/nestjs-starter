...
import { AppWorker, DatabaseLock, JobName, JobStatus, QueueName, StorageFolder } from 'src/enum';
...
export class BackupService extends BaseService {
  ...
  async cleanupDatabaseBackups() {
    ...
    const failedBackups = files.filter((file) => file.match(/db-backup-\d+\.sql\.gz\.tmp$/));
    const backups = files
      .filter((file) => file.match(/db-backup-\d+\.sql\.gz$/))
      .sort()
      .reverse();
